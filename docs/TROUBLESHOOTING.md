...
`setup/03_cuda_wheels.ps1` 도 이제 `o_voxel_vb` 를 함께 설치하고,
`setup/doctor.py` 의 G2 게이트가 `postprocess` 서브모듈 유무를 검사한다.
처음에는 손으로 고쳤을 뿐 스크립트에 반영하지 않아, 클린 재설치를 하면
같은 자리에서 다시 죽는 상태였다 — S7 검증에서 잡았다.
...
`setup/03_cuda_wheels.ps1` 이 이제 이 휠을 설치하고, `doctor.py --gate g2` 가
`import nvdiffrast.torch` 를 검사한다. 위 `o_voxel_vb` 와 마찬가지로 손으로만
고쳐둔 상태였고, S7 검증에서 드러났다.
...
또한 `app.py`의 `generate_3d()`는 서버사이드 턴테이블 렌더를 호출한다.
이 프로젝트는 그 렌더를 쓰지 않는다 — 브라우저 3D 뷰어가 대신한다 (결정 D6).
nvdiffrast 자체는 설치한다. UV 텍스처 베이킹에 필요하기 때문이다(위 항목 참고).
...
```
1회차 — 진행률: timed out
2회차 — 전처리: HTTP 500: {"detail":"전처리에 실패했습니다."}
```

**실제로는 1회차가 멀쩡히 돌고 있었다.** GLB 추출의 `Parameterizing new mesh`
(cumesh 의 UV 아틀라스 생성) 구간이 C 확장 안에서 최대 4분을 보내며 GIL 을
오래 쥔다. 그동안 uvicorn 이벤트 루프가 밀려 `/api/progress` 응답이 30초를
넘긴다. 클라이언트가 이것을 실패로 접고 다음 건을 밀어 넣으면, 큐가 슬롯
하나뿐이라 새 전처리가 대기하다가 180초 한도에서 잘리고 — 그 대기가
'전처리 실패'로 기록된다. 한 건이 넘어졌는데 두 건이 실패로 남는다.

양쪽을 고쳤다.

- `tools/_api.py` `poll()` — 폴링 실패를 20회 '연속' 실패까지 견딘다.
  한 번이라도 성공하면 계수가 초기화된다.
- `server/main.py` `PREPROCESS_WAIT` — 180초 → 1200초. 그리고 한도를 넘겨도
  작업이 아직 `queued`/`running` 이면 실행을 실패로 못박지 않고 **503** 을
  돌려준다. 실패가 아니라 '지금은 바쁘다'이기 때문이다.
- `tools/run_batch.py` — 503 을 받으면 60초 뒤 최대 4회까지 다시 시도한다.

---

### 같은 seed 인데 결과가 매번 다르다 (S6)

**정상이다. 이 파이프라인은 비결정적이다.**

같은 이미지·같은 seed(42)·같은 해상도(1024)로 3회 돌린 결과:
...
원인을 생성 쪽으로 좁힐 수 있다. **`processed.png` 는 네 번 모두 비트 단위로
동일했다** — 배경 제거는 결정적이고, 따라서 생성에 들어가는 입력이 같다.
같은 입력·같은 seed 에서 출력이 갈리므로 비결정성은 CUDA 생성 경로에 있다.

`DETERMINISTIC=1` 로 cuDNN 알고리즘 자동선택과 TF32 를 끌 수 있다.
**하지만 실측 결과 듣지 않았다.**

| 모드 | 소요 | 정점 |
|---|---:|---:|
| 기본 | 178.5s / 215.3s | 795,792 / 797,445 |
| `DETERMINISTIC=1` | 346.5s / 370.5s | 797,546 / 815,286 |

재현되지 않는 것은 그대로이고 소요만 70% 늘었다. 남은 비결정성의 원인은
cuDNN 도 TF32 도 아니라는 뜻이다 — 희소 복셀 커널의 atomic 누적 순서가
유력하고, 그것은 이 스위치로 통제되지 않는다. **켜지 않는 편이 낫다.**

비교가 목적이라면 파일 해시 대신 **정점·삼각형 수와 육안**으로 본다.
기본 모드의 편차는 정점 기준 약 0.7% 로, 형상이 바뀌는 수준은 아니다.
