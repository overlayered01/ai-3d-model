...
**S7 검증이 실제로 결함 2건을 찾아냈다.** 손으로 고쳤을 뿐 설치 스크립트에
반영하지 않은 것들이라, 클린 재설치를 하면 같은 자리에서 다시 죽는 상태였다.

- `03_cuda_wheels.ps1` 이 `nvdiffrast` 를 설치하지 않았다 → GLB 추출에서 실패
- 같은 스크립트가 `o_voxel_vb` 대신 `_ap` 판만 설치했다 → `postprocess` 없음

둘 다 스크립트와 `doctor.py` G2 게이트에 반영했다.
