# Pixal3D 로컬 구축 계획서 (rev.3 — 구축 완료 반영)

- 작성일: 2026-08-22 (rev.3: 2026-08-23)
- 상태: **게이트 G0~G3 통과.** Track A(Windows 네이티브)로 GLB 생성 성공.
...
- 배치 CLI(`run_batch.py`)를 서버 API 호출로 구현 → `report.csv`
...
| R4 | **1536 표준 모드 OOM** (24GB 중 3.4GB 선점유) | 중 | **높음** | `--low_vram` + 1024 기본값. 웹 UI에서 해상도 선택 가능하게 |
...
| R12 | `EXT_texture_webp` 미지원 뷰어에서 텍스처 누락 | 중 | 중 | Phase 3에서 조기 확인. 실패 시 `extension_webp=False` |
